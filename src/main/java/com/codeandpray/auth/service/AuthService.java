...
import com.codeandpray.auth.dto.*;
...
public class AuthService {
    ...
    public AuthResponse register(RegisterRequest request) {
        ...
        if (request.password().getBytes(StandardCharsets.UTF_8).length > 72) {
            throw BusinessException.badRequest("Пароль должен занимать не более 72 байт UTF-8");
        }
        if (userRepository.existsByEmail(email)) {
            throw BusinessException.conflict("Пользователь с таким email уже существует");
        }
        User user = User.registerAthlete(email,
    ...
    public AuthResponse login(LoginRequest request) {
        if (request.password().getBytes(StandardCharsets.UTF_8).length > 72) {
            throw new BadCredentialsException("Некорректные учётные данные");
        }
