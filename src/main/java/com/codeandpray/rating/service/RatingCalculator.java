...
public class RatingCalculator {
    ...
    public Calculation calculate(RatingInput input) {
        if (input.qualificationBonus() < 0) {
            throw new IllegalArgumentException("Отрицательный бонус квалификации");
        }
        long sum = 0;
        Set<Long> ids = new HashSet<>();
        for (var result : input.results()) {
            if (result.resultId() <= 0 || !ids.add(result.resultId())
                    || result.ratingPoints() < 0 || result.formulaVersion() <= 0) {
                throw new IllegalArgumentException("Некорректные данные опубликованного результата");
            }
            sum = Math.addExact(sum, result.ratingPoints());
        }
        return new Calculation(sum, input.qualificationBonus(),
                Math.addExact(sum, input.qualificationBonus()));
    }
